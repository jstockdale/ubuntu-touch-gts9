...
revision, called **"rev"** throughout these docs: the digit after `XXU`/`XXS`
in a firmware build string (`X910XX`**`S5`**`CYG1` = rev 5), shown as
`RP SWREV` on the Download Mode screen. Flashing firmware with a higher rev
burns the fuse forward **permanently**; it can never be lowered again.
...
the fuse is what decides: a unit that took One UI 8.0 while still fuse rev 5
can be rolled back to One UI 7 and unlocked ([FLASHING.md](FLASHING.md)).
...
5G siblings (SM-X716B/X816B/X916B) are not supported, and the flash scripts
...
1. **Back up before first flash:** a TWRP full backup *including EFS*
   (TWRP install: [FLASHING.md](FLASHING.md)), raw dumps of
   `efs sec_efs persist optics prism up_param`, and an archived copy of
   your factory firmware package. Old builds rot off mirrors; your copy is
   the unit's permanent way back.
...
material), then come back and run
[`fixes/post-flash/gts9wifi-post-flash.sh`](fixes/post-flash/) for the
quality-of-life fixes (audio, S-Pen pointer, touchpad rotation). See
...
1. Verify your unit's rev, unlock, and install TWRP per
   [FLASHING.md](FLASHING.md); obtain the matching donor firmware,
   hash-verified per [FIRMWARE.md](FIRMWARE.md).
...
3. Build, from the repo root:
   ...
   ```
   Then follow [`devices/gts9uwifi/README.md`](devices/gts9uwifi/README.md)
...
carefully assembled but **unexecuted** port kit: build script, kernel import
bundle, firmware extractor, reference PIT/DTS, and a six-phase runbook. If
you have an unlockable unit and porting experience, you could be the first:
...
- Test reports for the untested cells in [FIRMWARE.md](FIRMWARE.md):
  rev 1 builds on any device, and the S9+ port.
- Open engineering problems ledgered in
  [PORT-STATE.md §6](docs/knowledge/PORT-STATE.md): sensors HAL and
...
- [`fixes/`](fixes/) – standalone installable fixes: `audio/`, `input-pen/`,
  `input-touchpad/`, `hygiene/` (apt pin and greeter fd-limit guards), and
...
third-party work (Samsung OSRC / Qualcomm GPL-2.0 kernel code and Azkali's
